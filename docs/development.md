# Lock Clock — Development

Build, packaging, private API notes, research findings, and test checklists. For end-user install steps, see [README.md](../README.md). For how to open a PR, see [CONTRIBUTING.md](../CONTRIBUTING.md).
...
The shared Debug scheme passes `--debug`, which opens the diagnostics window instead of Settings on launch.
...
1. In **System Settings → Wallpaper → Clock Appearance**, set **Show large clock** to **Never**, then run `sudo diskutil apfs updatePreboot /` and restart (see [README.md](../README.md)).
2. Launch `LockClock.app`. On first launch, **Settings** opens automatically.
3. Enable **Classic Lock Screen Clock (Launch at Login)** in Settings.
4. There is no Dock icon. A clock icon appears in the menu bar.
...
In **Settings**, turn on **Enable Classic Lock Screen Clock (Launch at Login)**. That registers the app with `SMAppService.mainApp` and enables the clock.
...
Bumps the build number in `Version.xcconfig` (`{major}.{minor}.{build}`), builds Release, and writes an unsigned zip to `dist/LockClock-1.0.N.zip`. Change `MAJOR` or `MINOR` in that file by hand when you want `1.1.x` or `2.0.x`.
...
| Screen Recording | No                                         |
| Full Disk Access | No                                         |
| Root             | No (except optional `updatePreboot`)       |
| Disable SIP      | No                                         |
...
- Visual lock-screen placement uses `SystemClockLayout` (time cap height ≈ 10.3% of screen height, date top ≈ 12.3% from top). Tune in `SystemClockLayout.swift` if needed on your display.
...
defaults write app.lockclock.LockClock clock.verticalFraction -float 0.20
defaults write app.lockclock.LockClock clock.showSeconds -bool false
```

Font family values are persisted as `system`, `lockScreen.<id>` (Rounded / Classic / Flip Up / New York / Stencil), or `family.<Installed Family Name>`.
